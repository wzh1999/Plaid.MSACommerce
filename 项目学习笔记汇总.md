...
1.增加了密码泄露风险，因为用户需要多次输入密码会受到攻击者攻击
2.服务器负担加重
3.难以实现细粒度的权限管理
...
##### 刷新令牌
刷新令牌==》获取新的访问令牌的凭证==》具有较长的有效期(允许用户在颁发的访问令牌过期后，通过刷新令牌的方式让用户在很长一段时间都有对应的令牌凭证)
1. 无缝的身份认证的体验--无需重新登录就可以获取到对应的新令牌
2. 降低安全风险

注意事项:刷新令牌的保护、刷新令牌的有效期(定期更换令牌)、避免滥用刷新令牌

实现流程

![1785414486414](assets/1785414486414.png)

Refresh Token:刷新Token信息---总的过期时间；总的令牌时间过期后将需要重新登录，一般时间比较长
Access Token:权限Token--过期时间短期的；当短期过期时刷新令牌时间未过期的时候则重启刷新Token

###### 实现步骤

~~~c#
/*删除app-AuthServer-Service服务原有颁发Token的逻辑
1.删除IIdentityService
2.删除IdentitySerfvice
3.删除IUserService*/
//新增调整成 刷新Token AuthToken逻辑
/*
1.增加一个接口ITokenService
2.增加一个具体的实现TokenService
*/
/// <summary>
/// 令牌服务接口 颁发 刷新 授权令牌接口
/// </summary>
...
//在入口增加依赖注入 在AuthServer中的DependencyInjection 扩展方法中增加
//注入认证的逻辑 
...
//JWT Token的逻辑
 ...
 {
     // services.AddSingleton<IIdentityService, IdentityService>(); //原有的认证授权逻辑
